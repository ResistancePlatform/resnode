...
const Resistance = require('./resistancecfg')

const resistancecfg = Resistance.getResistanceConfig()

const local = new LocalStorage('./config/local')
const cfg = {
  url: resistancecfg.url,
  ssl: {
    enabled: false
  },
  username: resistancecfg.rpcuser,
  password: resistancecfg.rpcpassword
}
  ...
  if (err.response && err.response.data) {
    if (typeof err.response.data === 'object') {
      ...
      if (err.response.data.error.code === -28) {
        console.log(logtime(), `Resistanced: Waiting - ${msg}`)
        return cb(msg, 'starting')
      }
      console.log(logtime(), `ERROR resistanced ${txt}  ${msg}`)
    } else {
      msg = err.response.data
      if (msg.indexOf('depth') !== -1) {
        console.log(logtime(), `ERROR resistanced rpc issue:  ${msg}`)
        return cb(msg, 'queue')
      }
      console.log(logtime(), `ERROR resistanced ${txt}  ${msg}`)
    }
  } else {
    console.log(logtime(), `ERROR resistanced ${txt}`)
  }
...
class SNode {
  constructor(rpc, cfgresistance) {
    this.rpc = rpc
    this.resistancecfg = cfgresistance
    ...
    this.memNearEnd = {}
    this.waiting = false
    this.resistanceDownInterval = 1000 * 61
    this.resistanceDownTimer = null
    this.resistanceDownLoop = () => {
  ...
  static auto() {
    ...
    return new SNode(rpc, resistancecfg)
  }
  ...
  checkResistance() {
    ...
    self.getPrimaryAddress((err) => {
      if (err) return
      console.log(logtime(), 'Resistance connected.')
      clearInterval(self.resistanceDownTimer)
      self.collectStats()
    })
  }
  ...
  getAddrWithBal(cb) {
    ...
    self.rpc.getinfo()
      ...
      .then((addrs) => {
        if (addrs.length === 0) {
          console.log('No private address found. Please create one using \'resistance-cli z_getnewaddress\' and send at least 0.04 RES for challenges split into 4 or more transactions')
          return cb(null)
        }
        const bals = []
  ...
  execChallenge(chal) {
    ...
    self.rpc.getblockhash(chal.blocknum)
      .then((hash) => {
        self.queueCount = 0
        if (self.resistanceDownTimer) clearInterval(self.resistanceDownTimer)
      ...
      .catch(err => rpcError(err, 'get block hash for challenge', (errmsg, errtype) => {
        ...
        if (errtype && errtype === 'starting') {
          ...
          resp.error = 'could not get block hash from resistanced'
        }
        ...
        // console.error(logtime(), error)
        if (!self.resistanceDownTimer) {
          self.resistanceDownTimer = setInterval(self.resistanceDownLoop, self.resistanceDownInterval)
        }
      }))
  }
    ...
    await self.rpc.z_getoperationstatus([opid])
      .then((operation) => {
        ...
        if (operation.length === 0) {
          ...
          resp.ident = self.ident
          self.socket.emit('chalresp', resp)
          console.log(logtime(), 'Challenge submit: failed. Could not find resistance operation.')
          console.log(logtime(), 'Clearing timer')
          clearInterval(self.opTimer)
          return
        }
        ...
        if (op.status === 'success') {
          ...
          self.rpc.z_getoperationresult([opid])
            .catch((err) => {
              console.log(logtime(), 'ERROR getoperationresult unable to get data from resistanced')
  ...
  getConfig(req, trkver, hw, nodejs, platform) {
    ...
    self.rpc.getinfo()
      .then((data) => {
        if (data.error) {
          console.log(logtime(), 'ERROR getConfig unable to get data from resistanced')
  ...
  collectStats() {
    ...
    self.getStats((err, stats) => {
      if (err) {
        ...
        if (!self.resistanceDownTimer) self.resistanceDownTimer = setInterval(self.resistanceDownLoop, self.resistanceDownInterval)
      } else {
        if (self.resistanceDownTimer) clearInterval(self.resistanceDownTimer)
        self.getTLSPeers((error, tlsPeers) => {
          if (error) console.log(logtime(), `Unable to get peers from resistance. ${error}`)
  ...
  getStats(cb) {
    const self = this
    if (self.waiting) return cb('Waiting for resistanced')
  ...
  getNetworks(req, cb) {
    ...
    self.rpc.getnetworkinfo()
      ...
      .catch((err) => {
        console.log(logtime(), 'ERROR: getNetworks  unable to get data from resistanced')
        console.error(logtime(), err.message, err.response.data)
        return cb(err.message)
      })
  }
  ...
  getTLSPeers(cb) {
    ...
    return self.rpc.getpeerinfo()
      ...
      .catch((err) => {
        console.log(logtime(), 'ERROR: getTLSPeers  unable to get data from resistanced')
